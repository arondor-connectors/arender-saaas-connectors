   ...
   YAHOO.extend(Alfresco.DocumentVersions, Alfresco.component.Base,
   {
      ...
      getDocumentVersionMarkup: function DocumentVersions_getDocumentVersionMarkup(doc)
      {
         var downloadURL = Alfresco.constants.PROXY_URI + 'api/node/content/' + doc.nodeRef.replace(":/", "") + '/' + doc.name + '?a=true',
            html = '';
			var showVersionUrl = this.getShowVersionUrl(doc.label);

         html += '<div class="version-panel-left">'
         html += '   <span class="document-version"><a href="' + showVersionUrl +'">' + $html(doc.label) + '</a></span>';
         html += '</div>';
         ...
         html += '   	<a href="#" name=".onCompareVersionClick" rel="' + doc.label + '" class="' + this.id + ' compare" title="' + this.msg("label.compareversion") + '">&nbsp;</a>';
         html += '   </span>';
	  ...
	  getShowVersionUrl: function DocumentVersions_getShowVersionUrl(version)
	  {
		var showVersionUrl =   window.location.href;
		var urlObject = Alfresco.util.parseURL(showVersionUrl);
		if( urlObject.queryParams.version )
		{
			var motif = "&version=" + urlObject.queryParams.version;
			showVersionUrl = showVersionUrl.replace(motif, "&version=" + version )
		}
		else{
			showVersionUrl =  showVersionUrl + "&version=" + version;
		}
		return showVersionUrl;
	  },
      ...
      /**
       * Called when a "onCompareVersionClick" link has been clicked for a version.
       * Will display the compare version dialog.
       *
       * @method onCompareVersionClick
       * @param version
       */
      onCompareVersionClick: function DocumentVersions_onCompareVersionClick(version)
      {
         // Show the dialog to select the version to compare against
         Alfresco.module.getCompareVersionInstance().show(
         {
            filename: this.latestVersion.name,
            nodeRef: this.options.nodeRef,
            version: version,
            onCompareVersionComplete:
            {
               fn: this.onCompareVersionComplete,
               scope: this
            }
         });
      },
      ...
      /**
       * Fired by the Compare Version component after a successful compare.
       * Will display a message and reload the page.
       *
       * @method onCompareVersionComplete
       */
      onCompareVersionComplete: function DocumentVersions_onCompareVersionComplete()
      {
         Alfresco.util.PopupManager.displayMessage(
         {
